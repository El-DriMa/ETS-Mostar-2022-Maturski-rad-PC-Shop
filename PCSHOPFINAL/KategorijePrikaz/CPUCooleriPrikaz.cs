...
{
    public partial class CPUCooleriPrikaz : Form
    {
        public CPUCooleriPrikaz()
        {
            InitializeComponent();
        }

        private void CPUCooleriPrikaz_Load(object sender, EventArgs e)
        {
            dgCPUCooleri.DataSource = KategorijeProizvodaDA.prikazCooleraGrid();
            ucitajProizvodjace();
        }
        ...
        private void btnPretraga_Click(object sender, EventArgs e)
        {
            ...
            {
                dgCPUCooleri.DataSource = PretrageDA.pretragaPoNazivuCPUCooleri(txtPretraga.Text);
            }
            else
            {
                dgCPUCooleri.DataSource = KategorijeProizvodaDA.prikazCooleraGrid();
        ...
        {
            if (dgCPUCooleri.Rows.Count != 0)
            {
                int proizvodID = Convert.ToInt32(dgCPUCooleri.SelectedRows[0].Cells[0].Value);
        ...
        private void btnObrisi_Click(object sender, EventArgs e)
        {
            ...
            if (dgCPUCooleri.Rows.Count != 0)
            {
                int id = Convert.ToInt32(dgCPUCooleri.SelectedRows[0].Cells[0].Value.ToString());
