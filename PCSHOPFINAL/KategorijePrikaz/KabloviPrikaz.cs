...
{
    public partial class KabloviPrikaz : Form
    {
        public KabloviPrikaz()
        {
            InitializeComponent();
        }

        private void KabloviPrikaz_Load(object sender, EventArgs e)
        {
            dgKablovi.DataSource = KategorijeProizvodaDA.prikazKablovaGrid();
            ucitajProizvodjace();
        }
        ...
        private void btnPretraga_Click(object sender, EventArgs e)
        {
            ...
            {
                dgKablovi.DataSource = PretrageDA.pretragaPoNazivuKablovi(txtPretraga.Text);
            }
            else
            {
                dgKablovi.DataSource = KategorijeProizvodaDA.prikazKablovaGrid();
        ...
        {
            if (dgKablovi.Rows.Count != 0)
            {
                int proizvodID = Convert.ToInt32(dgKablovi.SelectedRows[0].Cells[0].Value);
        ...
        private void btnObrisi_Click(object sender, EventArgs e)
        {
            if (dgKablovi.Rows.Count != 0)
            {
                int id = Convert.ToInt32(dgKablovi.SelectedRows[0].Cells[0].Value.ToString());
