...
{
    public partial class MikrofoniPrikaz : Form
    {
        public MikrofoniPrikaz()
        {
            InitializeComponent();
        }

        private void MikrofoniPrikaz_Load(object sender, EventArgs e)
        {
            dgMikrofoni.DataSource = KategorijeProizvodaDA.prikazMikrofonaGrid();
            ucitajProizvodjace();
        }
        ...
        private void btnPretraga_Click(object sender, EventArgs e)
        {
            ...
            {
                dgMikrofoni.DataSource = PretrageDA.pretragaPoNazivuMikrofoni(txtPretraga.Text);
            }
            else
            {
                dgMikrofoni.DataSource = KategorijeProizvodaDA.prikazMikrofonaGrid();
        ...
        {
            if (dgMikrofoni.Rows.Count != 0)
            {
                int proizvodID = Convert.ToInt32(dgMikrofoni.SelectedRows[0].Cells[0].Value);
        ...
        private void btnObrisi_Click(object sender, EventArgs e)
        {
            if (dgMikrofoni.Rows.Count != 0)
            {
                int id = Convert.ToInt32(dgMikrofoni.SelectedRows[0].Cells[0].Value.ToString());
